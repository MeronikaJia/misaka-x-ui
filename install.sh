...
download_xui(){
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/MeronikaJia/misaka-x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ -z "$last_version" ]]; then
            red "检测 x-ui 版本失败，可能是超出 Github API 限制，正在使用备用源检测最新版本"
            last_version=$(curl -sm8 https://raw.githubusercontents.com/MeronikaJia/misaka-x-ui/main/config/version)
            if [[ -z "$last_version" ]]; then
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(archAffix).tar.gz https://github.com/MeronikaJia/misaka-x-ui/releases/download/${last_version}/x-ui-linux-$(archAffix).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/MeronikaJia/misaka-x-ui/releases/download/${last_version}/x-ui-linux-$(archAffix).tar.gz"
    ...
    wget -N --no-check-certificate https://raw.githubusercontents.com/MeronikaJia/misaka-x-ui/main/x-ui.sh -O /usr/bin/x-ui
